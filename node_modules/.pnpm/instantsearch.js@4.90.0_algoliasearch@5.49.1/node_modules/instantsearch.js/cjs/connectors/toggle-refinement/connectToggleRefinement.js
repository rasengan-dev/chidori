 ...
 */ var connectToggleRefinement = function connectToggleRefinement(renderFn) {
    ...
    return function(widgetParams) {
        ...
        var on = toArray.toArray(userOn).map(escapeFacetValue.escapeFacetValue);
        var off = hasAnOffValue ? toArray.toArray(userOff).map(escapeFacetValue.escapeFacetValue) : undefined;
        var sendEvent;
        ...
        return {
            ...
            getWidgetRenderState: function getWidgetRenderState(param) {
                ...
                if (results) {
                    var offValue = toArray.toArray(off || false);
                    var allFacetValues = results.getFacetValues(attribute, {}) || [];
                    var onData = on.map(function(v) {
                        return find.find(allFacetValues, function(param) {
                            var escapedValue = param.escapedValue;
                            return escapedValue === escapeFacetValue.escapeFacetValue(String(v));
                        });
                    }).filter(function(v) {
                        return v !== undefined;
                    });
