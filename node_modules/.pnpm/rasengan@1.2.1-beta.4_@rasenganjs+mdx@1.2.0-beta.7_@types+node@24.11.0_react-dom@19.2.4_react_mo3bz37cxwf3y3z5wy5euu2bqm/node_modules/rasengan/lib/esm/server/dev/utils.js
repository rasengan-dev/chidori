 ...
 */
export async function logServerInfo(port, mode, open = false) {
    // Constants
    const arrowRight = '\u2192';
    // Spinner
    console.log('');
    ...
    let packageJson = await fs.readFile('node_modules/rasengan/package.json', 'utf-8');
    ...
    if (ipAddress) {
        process.stdout.write(`${chalk.bold.green(arrowRight)} ${chalk.bold('Network:')} ${chalk.blue(`http://${ipAddress}:${chalk.bold(port)}`)}\n`);
    }
    // Display options
 ...
 */
export function extractMetaFromRRContext(context) {
    ...
    const pageLoaderData = context.loaderData[pageLoaderId] ?? { meta: {} }; // This is the loader data specific to the page
    const layoutLoaderData = context.loaderData[layoutLoaderId] ?? { meta: {} }; // This is the loader data specific to the layout
 ...
 * @param context React Router context
 */
export function extractHeadersFromRRContext(context) {
...
export function isResourceRequest(request) {
    const accept = request.headers['accept'] || '';
    // Check common resource-related MIME types in the Accept header
    if (accept.includes('image/') || // Images
 ...
 */
export function isRedirectResponse(context) {
 ...
 */
export async function isStaticRedirectFromConfig(req, redirects) {
